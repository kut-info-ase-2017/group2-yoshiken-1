...
json_url = '/opt/lampp/htdocs/phpapi/member.json'
...
def setup():
	...
	member = pd.read_json(json_url, convert_dates=['lastin', 'lastout', 'totalStay'])
...
def out_upload(member, target):
	...
	member.loc[(member.name == str(target), 'totalStay')] = pd.to_timedelta(member.loc[(member.name == str(target), 'totalStay')].values[0]*1000000) + pd.to_timedelta(add_time)
